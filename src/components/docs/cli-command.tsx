...
export function CLICommand({ componentName, className }: CLICommandProps) {
    ...
    const getFullCommand = (pm: PackageManager) => {
        ...
        return `${config.command} shadcn@latest add "https://obsidianui.dev/r/${componentName}.json"`;
    };
    ...
    const renderHighlightedCommand = () => {
        ...
        return (
            <code className="text-sm font-mono leading-relaxed">
                ...
                {" "}
                <span className="text-amber-600 dark:text-amber-400">{`"https://obsidianui.dev/r/${componentName}.json"`}</span>
            </code>
        );
    };
